...
export default function LiquidGradient({ 
  ...
  return (
    <div className="liquid-container fixed inset-0 z-0 w-full h-full">
      <div ref={containerRef} className="liquid-canvas-wrapper absolute inset-0 w-full h-full" />
      
      {/* Custom Cursor */}
      <div 
        ref={cursorRef} 
        className={`cursor-ring absolute w-10 h-10 border-2 border-white rounded-full pointer-events-none transition-opacity duration-300 z-50 mix-blend-difference ${isDarkMode ? 'dark' : ''}`}
        style={{ opacity: showCursor ? 1 : 0 }} 
      />
      <div 
        ref={cursorDotRef} 
        className={`cursor-dot-element absolute w-2 h-2 bg-white rounded-full pointer-events-none transition-opacity duration-300 z-50 mix-blend-difference ${isDarkMode ? 'dark' : ''}`}
        style={{ opacity: showCursor ? 1 : 0 }} 
      />
    </div>
  );
