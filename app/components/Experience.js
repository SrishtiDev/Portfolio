...
export default function Experience() {
  return (
    <section id="experience" className="py-10 border-b border-zinc-800/80 scroll-mt-16">
      ...
      <div className="space-y-8">
        {experiences.map((exp, idx) => (
          <div
            key={idx}
            className="group relative rounded-xl border border-zinc-800/90 bg-zinc-950/60 p-5 sm:p-6 transition-all hover:border-zinc-700 hover:bg-zinc-900/40"
          >
            {/* Header info */}
            ...
            <ul className="space-y-3 mb-5">
              {exp.bullets.map((bullet, bIdx) => (
                <li key={bIdx} className="flex items-start gap-3 text-sm text-zinc-300 leading-relaxed">
                  <CheckCircle2 size={15} className="text-cyan-400 shrink-0 mt-1 opacity-80" />
                  <span>{bullet}</span>
                </li>
              ))}
            </ul>
            ...
            <div className="flex flex-wrap gap-1.5 pt-3 border-t border-zinc-900">
              {exp.tech.map((t, tIdx) => (
                <span
                  key={tIdx}
                  className="px-2.5 py-0.5 rounded-md bg-zinc-900 text-cyan-300 border border-zinc-800 text-xs font-mono font-medium"
                >
                  {t}
                </span>
              ))}
